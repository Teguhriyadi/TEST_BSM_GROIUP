...
@push('title')
    Setting Persyaratan Dokumen Pinjaman | BSM Koperasi
@endpush
    ...
    @if ($jenisList->count() === 0)
        <div class="card shadow mb-4">
            <div class="card-body text-center py-5">
                ...
                <p class="text-muted small mb-3">Tambahkan jenis pinjaman terlebih dahulu sebelum mensetting persyaratan
                    dokumen.</p>
                        ...
                        @foreach ($jenisList as $j)
                            ...
                            <a href="{{ route('setting-persyaratan-pinjaman.index', ['jenis_pinjaman_id' => $j->id]) }}"
                                ...
                                <div>
                                    ...
                                    <div class="small {{ $isActive ? 'opacity-75' : 'text-muted' }}">Tenor
                                        {{ $j->tenor_minimal ? number_format($j->tenor_minimal, 0, ',', '.') : '?' }} -
                                        {{ $j->tenor_maksimal ? number_format($j->tenor_maksimal, 0, ',', '.') : '?' }}
                                        bulan | Bunga {{ number_format($j->bunga_tahunan, 2, ',', '.') }}%</div>
                                </div>
                                <div>
                                    <span
                                        class="badge {{ $isActive ? 'bg-white text-orange' : 'bg-orange text-white' }} px-2 py-1 small">{{ $countDok }}
                                        dok</span>
                                </div>
                            </a>
                        @endforeach
                ...
                @if (!$selectedJenis)
                    <div class="card shadow">
                        <div class="card-body text-center py-5 text-muted">
                            <p class="mb-2">Pilih salah satu jenis pinjaman di panel kiri untuk mengatur persyaratan
                                dokumen.</p>
                        </div>
                    </div>
                @else
                    @php
                        $selectedIds = $selectedJenis->dokumenPersyaratan
                            ? $selectedJenis->dokumenPersyaratan
                                ->pluck('id')
                                ->map(fn($v) => (string) $v)
                                ->values()
                                ->all()
                            ...
                            <div class="card-body">
                                @if ($masterAktif->count() === 0)
                                    <div class="alert alert-warning mb-0 small">
                                        Belum ada Master Dokumen yang aktif. Silakan buat master dokumen terlebih dahulu di
                                        menu Master Dokumen.
                                        @haspermission('MASTER_DOKUMEN_CREATE')
                                            <br><a href="{{ route('master-dokumen.create') }}"
                                                class="fw-semibold text-warning text-decoration-underline">Tambah Master
                                                Dokumen</a>
                                        @endhaspermission
                                    </div>
                                @else
                                                ...
                                                @foreach ($masterAktif as $i => $md)
                                                    ...
                                                    <tr class="{{ $checked ? 'bg-orange-50' : '' }}">
                                                        <td class="text-center align-middle">
                                                            <div class="form-check m-0 p-0 d-flex justify-content-center">
                                                                <input class="form-check-input cekDok" type="checkbox"
                                                                    name="master_dokumen_ids[]" value="{{ $md->id }}"
                                                                    id="cek_{{ $md->id }}"
                                                                    @checked($checked)>
                                                            </div>
                                                        </td>
                                                        <td class="align-middle">
                                                            <label for="cek_{{ $md->id }}"
                                                                class="m-0 p-0 cursor-pointer fw-medium">{{ $md->kode_dokumen }}</label>
                                                        </td>
                                                        <td class="align-middle">
                                                            <label for="cek_{{ $md->id }}"
                                                                ...
                                                                @if (!empty($md->deskripsi))
                                                                    <div class="text-muted small mt-1">{{ $md->deskripsi }}
                                                                    </div>
                                                                @endif
                                                            </label>
                                                        </td>
                                                        ...
                                                        <td class="align-middle text-center">
                                                            <div
                                                                class="form-check form-switch m-0 p-0 d-flex justify-content-center">
                                                                <input class="form-check-input" type="checkbox"
                                                                    name="wajib[{{ $md->id }}]"
                                                                    id="wajib_{{ $md->id }}" value="1"
                                                                    @checked($isWajib)>
                                                                <label class="form-check-label d-none"
                                                                    for="wajib_{{ $md->id }}">Wajib</label>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                    ...
                                    <div class="small text-muted mt-3">
                                        ...
                                        Centang kolom kiri untuk memasukan dokumen ke daftar persyaratan jenis ini. Aktifkan
                                        switch Wajib untuk dokumen yang WAJIB diunggah sebelum pengajuan (bisa diberi tanda
                                        merah pada anggota yang belum upload).
                                    </div>
                                @endif
                            </div>
                        </div>
        ...
        (function() {
            ...
            if ($checkAll) {
                ...
                function refreshCheckAllState() {
                    ...
                    var checkedCount = $list.filter(function(c) {
                        return c.checked;
                    }).length;
