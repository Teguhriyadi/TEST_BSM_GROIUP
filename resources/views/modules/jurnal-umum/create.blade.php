...
@push('title', 'Tambah Jurnal Manual')
@push('page-modules')
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Tambah Jurnal Manual</h1>
    ...
    <div class="col-lg-12">
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex justify-content-between align-items-center">
                <h6 class="m-0 font-weight-bold text-primary">Form Jurnal Manual</h6>
                <a href="{{ route('jurnal-umum.index') }}" class="btn btn-outline-primary btn-sm">
                    <i class="bi bi-arrow-left"></i> Kembali
                </a>
            </div>
            <form method="POST" id="formJurnal" action="{{ route('jurnal-umum.store') }}">
                @csrf
                <div class="card-body">
                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                            <label for="tanggal_jurnal" class="form-label">Tanggal Jurnal <span class="text-danger">*</span></label>
                            <input type="date" class="form-control @error('tanggal_jurnal') is-invalid @enderror" id="tanggal_jurnal" name="tanggal_jurnal" value="{{ old('tanggal_jurnal', date('Y-m-d')) }}">
                        ...
                        <div class="col-md-8">
                            ...
                            <input type="text" class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" value="{{ old('keterangan') }}" placeholder="Contoh: Penyesuaian beban sewa bulan September">
                            @error('keterangan')
                            <div class="invalid-feedback"><small>{{ $message }}</small></div>
                            @enderror
                        </div>
                ...
                <div class="card-footer bg-light py-3 d-flex justify-content-end gap-2">
                    <a href="{{ route('jurnal-umum.index') }}" class="btn btn-outline-secondary">Batal</a>
                    <button type="submit" id="btnSimpan" class="btn btn-orange" disabled>
                        <i class="bi bi-save me-1"></i> Simpan Sebagai Draf
                    </button>
                </div>
            </form>
        </div>
...
(function(){
    ...
    function rowTemplate(i){
        var opts = '<option value="">-- Pilih Akun --</option>' + coaList.map(function(c){ return '<option value="'+c.id+'">'+c.label+'</option>'; }).join('');
        return '<tr data-index="'+i+'">' +
            ...
            '<td><input type="text" class="form-control" name="details['+i+'][keterangan]" placeholder="Opsional"></td>' +
            '<td class="text-end"><input type="number" min="0" step="0.01" class="form-control form-control-sm text-end input-debet" name="details['+i+'][debet]" value="0"></td>' +
            '<td class="text-end"><input type="number" min="0" step="0.01" class="form-control form-control-sm text-end input-kredit" name="details['+i+'][kredit]" value="0"></td>' +
    ...
    document.addEventListener('DOMContentLoaded', function(){
        tambahBaris(); tambahBaris(); hitung();
