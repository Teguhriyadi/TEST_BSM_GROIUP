...
@push('title', 'Tambah Simpanan')
@push('page-modules')
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Form Tambah Simpanan</h6>
    </div>
    <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="jenis_simpanan_id" id="jenis_simpanan_id" class="form-select select2 @error('jenis_simpanan_id') is-invalid @enderror">
                        ...
                        <option value="{{ $js->id }}" {{ old('jenis_simpanan_id') == $js->id ? 'selected' : '' }}>
                            {{ $js->nama_jenis }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" name="nominal" id="nominal" step="0.01" min="0" class="form-control @error('nominal') is-invalid @enderror" value="{{ old('nominal') }}" placeholder="Nominal simpanan (Rupiah)">
                    @error('nominal')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" name="saldo" id="saldo" step="0.01" min="0" class="form-control @error('saldo') is-invalid @enderror" value="{{ old('saldo', 0) }}" placeholder="Saldo akhir simpanan (Rupiah)">
                    @error('saldo')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                </div>
                <div class="col-md-12 mb-3">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="2" class="form-control @error('keterangan') is-invalid @enderror" placeholder="Catatan transaksi simpanan">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                </div>
