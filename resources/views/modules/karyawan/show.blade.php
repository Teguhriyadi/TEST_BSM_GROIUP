...
@push('title', 'Detail Karyawan')
@push('page-modules')

@php
    use Illuminate\Support\Facades\Auth;
    $bisaUbah = Auth::check() && Auth::user()->hasPermission('KARYAWAN_UPDATE');
    $bisaHapus = Auth::check() && Auth::user()->hasPermission('KARYAWAN_DELETE');
@endphp
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div>
        ...
        <div class="small text-muted mt-1">
            No. Karyawan: {{ $karyawan->no_anggota ?? '-' }}
            @if($karyawan->nik)
                &nbsp;•&nbsp; NIK: {{ $karyawan->nik }}
            @endif
        </div>
...
@php
    $statusA = $karyawan->status ?? 'nonaktif';
    $statusClassA = $statusA === 'aktif' ? 'success' : 'secondary';
    $statusDaftar = $karyawan->status_pendaftaran ?? 'tidak_perlu_verifikasi';
    ...
    <div class="col-lg-7">
        <div class="card shadow">
            <div class="card-header py-3 d-flex flex-wrap flex-row align-items-center justify-content-between gap-2 bg-light">
                <h6 class="m-0 font-weight-bold text-primary">Data Pribadi Karyawan</h6>
                <div class="d-flex flex-wrap align-items-center gap-2">
                    ...
                    @if($bisaUbah)
                    <a href="{{ route('karyawan.edit', $karyawan->id) }}" class="btn btn-sm btn-success">
                        <i class="bi bi-pencil-square me-1"></i> Ubah Data
                    </a>
                    @endif
                    <a href="{{ route('karyawan.index') }}" class="btn btn-sm btn-outline-primary">
                        <i class="bi bi-arrow-left me-1"></i> Daftar Karyawan
                    </a>
                </div>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <div class="fw-semibold text-gray-800">{{ $karyawan->nama ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">No. Anggota</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->no_anggota ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">NIK</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->nik ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Jenis Kelamin</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->jenis_kelamin ? ucfirst($karyawan->jenis_kelamin) : '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Tempat, Tanggal Lahir</div>
                        <div class="fw-semibold text-gray-800">
                            {{ ($karyawan->tempat_lahir ?? '') . ($karyawan->tempat_lahir && $karyawan->tgl_lahir ? ', ' : '') . ($karyawan->tgl_lahir ? \Carbon\Carbon::parse($karyawan->tgl_lahir)->isoFormat('DD MMMM YYYY') : '-') }}
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Pekerjaan</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->pekerjaan ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Nomor HP</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->no_hp ?? $karyawan->user?->nomor_hp ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Email</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->user?->email ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Agama</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->agama ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Status Perkawinan</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->status_perkawinan ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Nama Ibu Kandung</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->nama_ibu ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Pendidikan Terakhir</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->pendidikan_terakhir ?? '-' }}</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Tanggal Registrasi</div>
                        <div class="fw-semibold text-gray-800">@tanggal($karyawan->created_at ?? '-')</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Tanggal Menjadi Anggota</div>
                        <div class="fw-semibold text-gray-800">@tanggal($karyawan->tgl_anggota ?? null)</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Cabang</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->cabang?->nama_cabang ?? '-' }} ({{ $karyawan->cabang?->kode_cabang ?? '-' }})</div>
                    </div>
                    <div class="col-12">
                        <div class="small text-muted">Alamat Lengkap</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->alamat ?? '-' }}</div>
                    </div>
                </div>
                @if($karyawan->status_pendaftaran === 'ditolak' || $karyawan->status_pendaftaran === 'disetujui')
                <hr class="my-3">
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="small text-muted">Tanggal Verifikasi Pendaftaran</div>
                        <div class="fw-semibold text-gray-800">@tanggal($karyawan->tgl_verifikasi_pendaftaran ?? null)</div>
                    </div>
                    <div class="col-12">
                        <div class="small text-muted">Catatan Verifikasi</div>
                        <div class="fw-semibold text-gray-800">{{ $karyawan->catatan_verifikasi_pendaftaran ?? '-' }}</div>
                    </div>
                </div>
                @endif
            </div>
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="fw-semibold text-gray-800">
                            @if(($karyawan->user?->force_change_password ?? 0) == '1' || ($karyawan->user?->force_change_password ?? false))
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="fw-semibold text-gray-800">@tanggalWaktu($karyawan->user?->last_login_at ?? null)</div>
                    </div>
                    <div class="col-md-6">
                        <div class="small text-muted">Password Diubah</div>
                        <div class="fw-semibold text-gray-800">@tanggalWaktu($karyawan->user?->password_changed_at ?? null)</div>
                    </div>
                </div>
                <hr class="my-3">
                <div class="row g-3">
