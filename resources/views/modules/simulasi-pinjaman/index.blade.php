...
@push('title')
<title>Simulasi Pinjaman | BSM Koperasi</title>
@endpush
            ...
            <form action="{{ route('simulasi-pinjaman.index') }}" method="GET">
                ...
                <div class="card shadow mb-4 border-left-orange">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-12">
                                <label class="form-label fw-medium small">Jenis Pinjaman <span class="text-danger">*</span></label>
                                <select id="jenis_pinjaman_id" name="jenis_pinjaman_id" class="form-select select2-advanced" required data-placeholder="Pilih jenis pinjaman">
                                    <option value="">-- Pilih Jenis Pinjaman --</option>
                                    @foreach($jenisPinjaman as $jp)
                                        <option value="{{ $jp->id }}"
                                            @selected((string) $selectedJenisId === (string) $jp->id)
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-medium small">Jumlah Pinjaman (Rp) <span class="text-danger">*</span></label>
                                <input type="number" id="jumlah_pinjaman" name="jumlah_pinjaman" min="0" step="100000" class="form-control"
                                    value="{{ old('jumlah_pinjaman', $jumlahPinjaman > 0 ? number_format($jumlahPinjaman, 0, '.', '') : '') }}"
                                    placeholder="Contoh: 10000000">
                                <div class="form-text small text-muted">Tidak boleh melebihi plafon maksimal jenis pinjaman.</div>
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-medium small">Tenor (Bulan) <span class="text-danger">*</span></label>
                                <input type="number" id="tenor" name="tenor" min="1" step="1" class="form-control"
                                    value="{{ old('tenor', $tenor > 0 ? $tenor : '') }}"
                                    placeholder="Contoh: 24">
                                <div class="form-text small text-muted" id="tenorHint">Pilih jenis pinjaman terlebih dahulu untuk melihat rentang tenor.</div>
                                @error('tenor')
                                    <div class="small text-danger mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" id="bunga_tahunan" class="form-control bg-light" readonly placeholder="Auto dari jenis pinjaman">
                                <div class="form-text small text-muted">Diambil otomatis dari pengaturan jenis pinjaman.</div>
                            </div>

                            <div class="col-md-6">
                                <label class="form-label fw-medium small">Plafon Maksimum Jenis Ini</label>
                                <input type="text" id="plafon_maksimal" class="form-control bg-light" readonly placeholder="Auto dari jenis pinjaman">
                            </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" id="angsuran_per_bulan" class="form-control bg-orange-50 text-gray-900 fw-semibold fs-5 border-orange" readonly placeholder="Hasil perhitungan otomatis">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer bg-light d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-sm btn-orange">
                            <i class="bi bi-calculator me-1"></i> Hitung (Server)
                        </button>
                    </div>
                </div>
            </form>
            ...
            <div class="card shadow mb-4 h-100 border-left-info">
                ...
                <div class="card-body">
                    ...
                    <div class="row g-2 small">
                        ...
                        <div class="col-6 text-end fw-semibold text-orange" id="resJumlah">{{ $h ? 'Rp ' . number_format($h['jumlah_pinjaman'], 0, ',', '.') : '-' }}</div>

                        <div class="col-6 text-muted">Tenor</div>
                        <div class="col-6 text-end fw-semibold" id="resTenor">{{ $h ? number_format($h['tenor_bulan'], 0, ',', '.') . ' bulan' : '-' }}</div>

                        <div class="col-6 text-muted">Bunga / Tahun</div>
                        <div class="col-6 text-end fw-semibold" id="resBunga">{{ $h ? number_format($h['bunga_tahunan'], 2, ',', '.') . '%' : '-' }}</div>

                        <div class="col-12 border-bottom my-2"></div>

                        <div class="col-6 text-muted">Angsuran Per Bulan</div>
                        <div class="col-6 text-end fw-bold text-orange fs-5" id="resAngsuran">{{ $h ? 'Rp ' . number_format($h['angsuran_per_bulan'], 0, ',', '.') : '-' }}</div>

                        <div class="col-6 text-muted">Total Bayar Selama Tenor</div>
                        <div class="col-6 text-end fw-semibold" id="resTotal">{{ $h ? 'Rp ' . number_format($h['total_bayar'], 0, ',', '.') : '-' }}</div>

                        <div class="col-6 text-muted">Total Bunga Dibayar</div>
                        <div class="col-6 text-end fw-semibold text-info" id="resBungaTotal">{{ $h ? 'Rp ' . number_format($h['total_bunga'], 0, ',', '.') : '-' }}</div>

                        <div class="col-6 text-muted">Rasio Bunga / Pokok</div>
                        <div class="col-6 text-end fw-semibold" id="resRasio">{{ $h && ($h['jumlah_pinjaman'] ?? 0) > 0 ? number_format(($h['total_bunga'] / $h['jumlah_pinjaman']) * 100, 2, ',', '.') . '%' : '-' }}</div>
                    </div>
                    ...
                    <div class="mt-4 p-3 bg-light rounded small">
                        ...
                        <div class="text-muted">Perhitungan menggunakan metode anuitas (bunga efektif). Angsuran per bulan bersifat flat. Nilai real dapat berbeda karena faktor pembulatan, hari libur, dan denda keterlambatan (jika ada).</div>
                    </div>
                </div>
            </div>
        </div>
...
(function () {
    ...
    function hitungSimulasi() {
        ...
        document.getElementById('plafon_maksimal').value = plafonMax > 0 ? 'Rp ' + numberFormat(plafonMax) : 'Tidak dibatasi';
        document.getElementById('tenorHint').textContent = 'Rentang diizinkan: minimal ' + tenMin + ' bulan, maksimal ' + tenMax + ' bulan.';
        ...
        document.getElementById('resBungaTotal').textContent = totalBunga > 0 ? 'Rp ' + numberFormat(totalBunga) : '-';
    ...
    function clearAll() {
        ...
        document.getElementById('tenorHint').textContent = 'Pilih jenis pinjaman terlebih dahulu untuk melihat rentang tenor.';
    ...
    function bind() {
        ...
        if ($jp) {
            $jp.addEventListener('change', function () { hitungSimulasi(); });
            if (window.jQuery) {
                jQuery($jp).on('select2:select select2:clear select2:unselect', function () { setTimeout(hitungSimulasi, 20); });
            }
        }
        ...
        if ($reset) {
            $reset.addEventListener('click', function () {
                if ($jp) { $jp.value = ''; if (window.jQuery) jQuery($jp).trigger('change').trigger('select2:clear'); }
                if ($jml) $jml.value = '';
                if ($tn) $tn.value = '';
                clearAll();
            });
