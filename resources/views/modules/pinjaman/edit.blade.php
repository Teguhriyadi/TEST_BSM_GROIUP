            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="anggota_id" id="anggota_id" class="form-select select2 @error('anggota_id') is-invalid @enderror">
                        ...
                        <option value="{{ $a->id }}" {{ old('anggota_id', $pinjaman->anggota_id) == $a->id ? 'selected' : '' }}>
                            {{ $a->no_anggota }} - {{ $a->nama }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="cabang_id" id="cabang_id" class="form-select select2 @error('cabang_id') is-invalid @enderror">
                        ...
                        @foreach($cabang as $c)
                        <option value="{{ $c->id }}" {{ old('cabang_id', $pinjaman->cabang_id) == $c->id ? 'selected' : '' }}>
                            {{ $c->nama_cabang }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <textarea name="tujuan" id="tujuan" rows="3" class="form-control @error('tujuan') is-invalid @enderror" placeholder="Tujuan penggunaan pinjaman">{{ old('tujuan', $pinjaman->tujuan) }}</textarea>
                    @error('tujuan')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select name="status" id="status" class="form-select select2 @error('status') is-invalid @enderror">
                        <option value="diajukan" {{ old('status', $pinjaman->status) == 'diajukan' ? 'selected' : '' }}>Diajukan</option>
                        <option value="diverifikasi" {{ old('status', $pinjaman->status) == 'diverifikasi' ? 'selected' : '' }}>Diverifikasi</option>
                        <option value="disetujui" {{ old('status', $pinjaman->status) == 'disetujui' ? 'selected' : '' }}>Disetujui</option>
                        <option value="ditolak" {{ old('status', $pinjaman->status) == 'ditolak' ? 'selected' : '' }}>Ditolak</option>
                        <option value="dicairkan" {{ old('status', $pinjaman->status) == 'dicairkan' ? 'selected' : '' }}>Dicairkan</option>
                        <option value="berjalan" {{ old('status', $pinjaman->status) == 'berjalan' ? 'selected' : '' }}>Berjalan</option>
                        <option value="lunas" {{ old('status', $pinjaman->status) == 'lunas' ? 'selected' : '' }}>Lunas</option>
                        <option value="dibatalkan" {{ old('status', $pinjaman->status) == 'dibatalkan' ? 'selected' : '' }}>Dibatalkan</option>
                    </select>
                    @error('status')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="date" name="tgl_pengajuan" id="tgl_pengajuan" class="form-control @error('tgl_pengajuan') is-invalid @enderror" value="{{ old('tgl_pengajuan', is_object($pinjaman->tgl_pengajuan) ? $pinjaman->tgl_pengajuan->format('Y-m-d') : ($pinjaman->tgl_pengajuan ? date('Y-m-d', strtotime($pinjaman->tgl_pengajuan)) : date('Y-m-d'))) }}">
                    @error('tgl_pengajuan')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <input type="date" name="tgl_cair" id="tgl_cair" class="form-control @error('tgl_cair') is-invalid @enderror" value="{{ old('tgl_cair', is_object($pinjaman->tgl_cair) ? $pinjaman->tgl_cair->format('Y-m-d') : ($pinjaman->tgl_cair ? date('Y-m-d', strtotime($pinjaman->tgl_cair)) : '')) }}">
                    @error('tgl_cair')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                </div>
