...
@push('title', 'Edit Pinjaman')
@push('page-modules')
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Form Edit Pinjaman</h6>
        </div>
        <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="anggota_id" id="anggota_id"
                            class="form-select select2 @error('anggota_id') is-invalid @enderror">
                            <option value="">-- Pilih Anggota --</option>
                            @foreach ($anggota as $a)
                                <option value="{{ $a->id }}"
                                    {{ old('anggota_id', $pinjaman->anggota_id) == $a->id ? 'selected' : '' }}>
                                    {{ $a->no_anggota }} - {{ $a->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="cabang_id" id="cabang_id"
                            class="form-select select2 @error('cabang_id') is-invalid @enderror">
                            <option value="">-- Pilih Cabang --</option>
                            @foreach ($cabang as $c)
                                <option value="{{ $c->id }}"
                                    {{ old('cabang_id', $pinjaman->cabang_id) == $c->id ? 'selected' : '' }}>
                                    {{ $c->nama_cabang }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="jenis_pinjaman_id" id="jenis_pinjaman_id"
                            class="form-select select2 @error('jenis_pinjaman_id') is-invalid @enderror"
                            data-auto-fill-targets="true">
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="jumlah_pinjaman" class="form-label">Jumlah Pinjaman <span
                                class="text-danger">*</span></label>
                        <input type="number" name="jumlah_pinjaman" id="jumlah_pinjaman" step="0.01" min="0"
                            class="form-control @error('jumlah_pinjaman') is-invalid @enderror"
                            value="{{ old('jumlah_pinjaman', $pinjaman->jumlah_pinjaman) }}"
                            placeholder="Jumlah pinjaman diajukan (Rupiah)">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" name="tenor" id="tenor" min="1" step="1"
                            class="form-control @error('tenor') is-invalid @enderror"
                            value="{{ old('tenor', $pinjaman->tenor) }}"
                            placeholder="Pilih jenis pinjaman terlebih dahulu">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="input-group">
                            <input type="number" id="bunga_display" step="0.01" min="0" max="100"
                                class="form-control bg-light" value="{{ old('bunga', $pinjaman->bunga) }}" readonly
                                disabled placeholder="Pilih jenis pinjaman terlebih dahulu">
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="angsuran_display" class="form-label">Angsuran Per Bulan <span
                                class="text-danger">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text">Rp</span>
                            <input type="number" id="angsuran_display" step="0.01" min="0"
                                class="form-control bg-light"
                                value="{{ old('angsuran_per_bulan', $pinjaman->angsuran_per_bulan) }}" readonly disabled
                                placeholder="Otomatis dihitung sistem">
                        </div>
                        <input type="hidden" name="angsuran_per_bulan" id="angsuran_per_bulan"
                            value="{{ old('angsuran_per_bulan', $pinjaman->angsuran_per_bulan) }}">
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="tujuan" id="tujuan" rows="3" class="form-control @error('tujuan') is-invalid @enderror"
                            placeholder="Tujuan penggunaan pinjaman">{{ old('tujuan', $pinjaman->tujuan) }}</textarea>
                        @error('tujuan')
                            <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                        @enderror
                    </div>
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="diajukan"
                                {{ old('status', $pinjaman->status) == 'diajukan' ? 'selected' : '' }}>Diajukan</option>
                            <option value="diverifikasi"
                                {{ old('status', $pinjaman->status) == 'diverifikasi' ? 'selected' : '' }}>Diverifikasi
                            </option>
                            <option value="disetujui"
                                {{ old('status', $pinjaman->status) == 'disetujui' ? 'selected' : '' }}>Disetujui</option>
                            <option value="ditolak" {{ old('status', $pinjaman->status) == 'ditolak' ? 'selected' : '' }}>
                                Ditolak</option>
                            <option value="dicairkan"
                                {{ old('status', $pinjaman->status) == 'dicairkan' ? 'selected' : '' }}>Dicairkan</option>
                            <option value="berjalan"
                                {{ old('status', $pinjaman->status) == 'berjalan' ? 'selected' : '' }}>Berjalan</option>
                            <option value="lunas" {{ old('status', $pinjaman->status) == 'lunas' ? 'selected' : '' }}>
                                Lunas</option>
                            <option value="dibatalkan"
                                {{ old('status', $pinjaman->status) == 'dibatalkan' ? 'selected' : '' }}>Dibatalkan
                            </option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="date" name="tgl_pengajuan" id="tgl_pengajuan"
                            ...
                            value="{{ old('tgl_pengajuan', is_object($pinjaman->tgl_pengajuan) ? $pinjaman->tgl_pengajuan->format('Y-m-d') : ($pinjaman->tgl_pengajuan ? date('Y-m-d', strtotime($pinjaman->tgl_pengajuan)) : date('Y-m-d'))) }}">
                        @error('tgl_pengajuan')
                            <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                        @enderror
                    </div>
                    @if (!auth()->user()->hasRole('Anggota') && !auth()->user()->hasRole('Karyawan'))
                        <div class="col-md-4 mb-3">
                            <label for="tgl_cair" class="form-label">Tgl Cair</label>
                            <input type="date" name="tgl_cair" id="tgl_cair"
                                class="form-control @error('tgl_cair') is-invalid @enderror"
                                value="{{ old('tgl_cair', is_object($pinjaman->tgl_cair) ? $pinjaman->tgl_cair->format('Y-m-d') : ($pinjaman->tgl_cair ? date('Y-m-d', strtotime($pinjaman->tgl_cair)) : '')) }}">
                            @error('tgl_cair')
                                <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                            @enderror
                        </div>
    ...
    <script>
        (function() {
            ...
            function renderDokumenPanel(dokList) {
                ...
                var wajibCount = dokList.filter(function(d) {
                    return d.is_wajib;
                }).length;
                ...
                var summaryHtml =
                    ...
                    '    <div class="small">' +
                    (wajibCount > 0 ? '<span class="badge bg-orange text-white py-0.5 px-2 me-1">Wajib ' + wajibCount +
                        '</span>' : '') +
                    (opsCount > 0 ? '<span class="badge bg-secondary text-white py-0.5 px-2">Opsional ' + opsCount +
                        '</span>' : '') +
                    '    </div>' +
                    '  </div>' +
                ...
                dokList.forEach(function(d, idx) {
                    ...
                    summaryHtml +=
                        ...
                        '    <span class="fw-semibold text-gray-800">' + (idx + 1) + '. ' + (d.nama ||
                            'Dokumen') + '</span>' + badge +
                        (d.kode ? ' <small class="text-muted">(' + (d.kode) + ')</small>' : '') +
                        '  </div>' +
                        (d.deskripsi ? '  <div class="text-muted small">' + d.deskripsi + '</div>' : '') +
                        (formatTxt ? '  <div class="text-muted small">Format yang diperbolehkan: ' + formatTxt +
                            '</div>' : '') +
                        '</li>';
                });
                summaryHtml += '</ul>';
                summaryHtml +=
                    ...
                    '      Unggah / perbarui dokumen di halaman detail pengajuan pinjaman. Dokumen wajib harus disetujui sebelum pengajuan bisa diverifikasi ke tahap selanjutnya.' +
                    '    </div>';
                summaryHtml += '  </div></div>';
            ...
            function hitungDanSetAngsuran() {
                ...
                if (opt && opt.value) {
                    ...
                    if (plafon > 0 && jumlah > plafon) {
                        ...
                        plafonAlert.textContent = '⚠ Jumlah pinjaman melebihi plafon maksimal jenis ini: ' + formatRp(
                            plafon);
                    } else {
            ...
            function isiDariJenis() {
                ...
                if (tenorRangeHint) {
                    tenorRangeHint.textContent = 'Rentang diizinkan: ' + tMin + ' - ' + tMaks +
                        ' bulan (bisa diubah manual dalam rentang ini).';
                }
                if (jpHint) {
                    jpHint.textContent =
            ...
            if (jpSel) {
                jpSel.addEventListener('change', function() {
                    isiDariJenis();
                });
                if (window.jQuery && window.$) {
                    $(jpSel).on('change select2:select select2:clear', function() {
                        isiDariJenis();
                    });
                }
            }
