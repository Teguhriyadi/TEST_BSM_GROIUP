...
@push('title', 'Edit Pembayaran Angsuran')
@push('page-modules')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="angsuran_id" id="angsuran_id" class="form-select select2 @error('angsuran_id') is-invalid @enderror">
                        ...
                        <option value="{{ $a->id }}" {{ old('angsuran_id', $pembayaranAngsuran->angsuran_id) == $a->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" name="tanggal_bayar" id="tanggal_bayar" class="form-control @error('tanggal_bayar') is-invalid @enderror" value="{{ old('tanggal_bayar', is_object($pembayaranAngsuran->tanggal_bayar) ? $pembayaranAngsuran->tanggal_bayar->format('Y-m-d') : date('Y-m-d', strtotime($pembayaranAngsuran->tanggal_bayar))) }}">
                    @error('tanggal_bayar')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" name="jumlah_bayar" id="jumlah_bayar" step="0.01" min="0" class="form-control @error('jumlah_bayar') is-invalid @enderror" value="{{ old('jumlah_bayar', $pembayaranAngsuran->jumlah_bayar) }}" placeholder="Jumlah yang dibayar anggota (Rupiah)">
                    @error('jumlah_bayar')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <select name="metode_pembayaran" id="metode_pembayaran" class="form-select select2 @error('metode_pembayaran') is-invalid @enderror">
                        <option value="tunai" {{ old('metode_pembayaran', $pembayaranAngsuran->metode_pembayaran) == 'tunai' ? 'selected' : '' }}>Tunai</option>
                        <option value="transfer" {{ old('metode_pembayaran', $pembayaranAngsuran->metode_pembayaran) == 'transfer' ? 'selected' : '' }}>Transfer</option>
                        <option value="lainnya" {{ old('metode_pembayaran', $pembayaranAngsuran->metode_pembayaran) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="dibayar_oleh" id="dibayar_oleh" class="form-select select2 @error('dibayar_oleh') is-invalid @enderror">
                        ...
                        @foreach($users as $u)
                        <option value="{{ $u->id }}" {{ old('dibayar_oleh', $pembayaranAngsuran->dibayar_oleh) == $u->id ? 'selected' : '' }}>
                            {{ $u->nama }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="2" class="form-control @error('keterangan') is-invalid @enderror" placeholder="Catatan bukti transfer / pembayaran (opsional)">{{ old('keterangan', $pembayaranAngsuran->keterangan) }}</textarea>
                    @error('keterangan')
                    <div class="invalid-feedback d-block"><small>{{ $message }}</small></div>
                    @enderror
                </div>
