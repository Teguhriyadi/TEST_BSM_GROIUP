...
@push('title')
<title>Master Dokumen Persyaratan | BSM Koperasi</title>
@endpush
...
<div class="container-fluid py-4">
    @include('modules.layouts.components.module-filter-card', [
        'showTanggalFilter' => false,
        'showCabangFilter' => false,
        'extraFilterSlot' => view()->make('modules.master-dokumen._filter_extra', get_defined_vars())->render()
    ])
            ...
            @else
                                        ...
                                        @hasanypermission(['MASTER_DOKUMEN_VIEW','MASTER_DOKUMEN_UPDATE','MASTER_DOKUMEN_DELETE'])
                                            <div class="d-flex justify-content-center gap-1">
                                                @haspermission('MASTER_DOKUMEN_VIEW')
                                                    <a href="{{ route('master-dokumen.show', $md->id) }}" class="btn btn-sm btn-info text-white text-xs px-2" title="Detail">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                @endhaspermission
                                                ...
                                                @haspermission('MASTER_DOKUMEN_DELETE')
                                                    <form onsubmit="return confirm('Hapus master dokumen ini?');" action="{{ route('master-dokumen.destroy', $md->id) }}" method="POST" class="d-inline m-0 p-0">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-danger text-white text-xs px-2" title="Hapus">
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    <div class="small text-muted">
                        Menampilkan {{ $masterDokumen->firstItem() ?? 0 }} - {{ $masterDokumen->lastItem() ?? 0 }} dari {{ $masterDokumen->total() ?? 0 }} data
                    </div>
                    <div>
